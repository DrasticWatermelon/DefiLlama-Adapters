...
Object.keys(config).forEach(chain => {
	...
	module.exports[chain] = {
		tvl: async (api) => {
			...
			// Print every (bond, collateralToken) pair
			// console.log(allBonds.map((bond, i) => [bond, collateralTokens[i]]))

			// Write every (boind, collateralToken) pair to a file
			// const fs = require('fs');
			// fs.writeFileSync('bonds.txt', allBonds.map((bond, i) => [bond, collateralTokens[i]]).join('\n'))
